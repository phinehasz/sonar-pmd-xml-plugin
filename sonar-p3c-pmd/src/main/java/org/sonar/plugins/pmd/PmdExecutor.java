...
public class PmdExecutor implements BatchExtension {
	...
	private Report executePmd(URLClassLoader classLoader) {
		...
		RuleContext xmlContext = new RuleContext();
		xmlContext.setReport(report);
		xmlContext.setLanguageVersion(new XmlLanguageModule().getVersion(""));
		executeXmlRules(pmdFactory, xmlContext, xmlFiles(Type.MAIN), PmdConstants.XML_REPOSITORY_KEY);
	...
	public Iterable<File> xmlFiles(Type fileType) {
		FilePredicates predicates = fs.predicates();
		return fs.files(predicates.and(
				predicates.hasLanguage(Xml.KEY),
				predicates.hasType(fileType)));
	}
	...
	public void executeXmlRules(PmdTemplate pmdFactory, RuleContext ruleContext, Iterable<File> files, String repositoryKey) {
		if (Iterables.isEmpty(files)) {
			// Nothing to analyze
			return;
		}
